...
def draw_list(items, on_click_func):
    ...
    for i, (symbol, color, title, desc, key) in enumerate(items):
        ...
        card = canvas.create_rectangle(list_x1, y1, list_x2, y2, fill="#202024", outline="#34343a", width=1)
        ...
        title_text = canvas.create_text(list_x1 + 115, y1 + 25, text=title, fill="white", font=("Arial", 19, "bold"), anchor="w")
        desc_text = canvas.create_text(list_x1 + 115, y1 + 52, text=desc, fill="#bdbdbd", font=("Arial", 12), anchor="w")
        arrow = canvas.create_text(list_x2 - 38, y1 + 37, text="›", fill="#cfcfcf", font=("Arial", 42, "bold"))
...
def show_home():
    ...
    canvas.create_text(width // 2, int(height * 0.12), text="HAMZA SERVICES", fill="#f2f2f2", font=("Arial", 52, "bold"))
    ...
    for index, (key, label) in enumerate(items):
        ...
        text_id = canvas.create_text(x, text_y, text=label, fill="#f2f2f2", font=("Arial", 32, "bold"), justify="center")
...
def draw_save_button(text, y, command):
    ...
    btn = canvas.create_rectangle(width // 2 - 130, y, width // 2 + 130, y + 55, fill="#2f7df6", outline="#2f7df6")
    txt = canvas.create_text(width // 2, y + 28, text=text, fill="white", font=("Arial", 18, "bold"))
...
def show_account_form(key):
    ...
    if key == "change_username":
        ...
        canvas.create_text(width // 2, 270, text="سيتم تفعيل شاشة القفل لاحقًا عند إضافة تسجيل الدخول.", fill="white", font=("Arial", 22, "bold"))
...
def draw_back_button(command):
    ...
    btn = canvas.create_rectangle(width // 2 - 90, height - 82, width // 2 + 90, height - 32, fill="#404040", outline="#666666", width=1)
    txt = canvas.create_text(width // 2, height - 57, text="رجوع", fill="white", font=("Arial", 18, "bold"))
